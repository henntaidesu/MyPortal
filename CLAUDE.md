...
一个「系统主页导航 + 单点登录」：前端是导航卡片页（Vue 3 + Vite，9920），后端是认证中心
...
# 后端：必须在 server/ 目录下执行，代码用的是相对包 app
cd server
...
账号 / 业务系统管理都在 `server/manage.py`：`users` `adduser` `passwd` `disable` `enable`
...
**本仓库没有测试、没有 linter、没有 CI**，也没有初始化 git。改动靠手动跑起来验证：
`examples/` 下三个示例系统（Python 8801 / Node 8802 / PHP 8803，后两个零依赖）
是验证 SSO 全链路最快的方式，用法见 [examples/README.md](examples/README.md)。
...
`StaticFiles` 在 [server/app/main.py](server/app/main.py) 里**挂在最后**，接口路由先匹配，
...
[server/app/db.py](server/app/db.py)：`sessions`（主页会话）、`tickets`（一次性票）、
...
- **`redirect_uri` 精确匹配白名单**（[clients.py](server/app/clients.py) `check_redirect_uri`）。
  改成前缀匹配或放行任意地址，票据就能被诱导送到别人服务器。
- **`next` 参数只接受站内路径**（前端 [auth.js](webside/src/auth.js) `safeNext`、
  后端 [sso.py](server/app/routers/sso.py) `authorize`）。放开就是开放重定向。
...
- **主页必须挂在域名根路径**：[NavCard.vue](webside/src/components/NavCard.vue) 里的 `href`
  是硬编码的绝对路径 `/sso/authorize`。挂子路径要改这里。
- **登录限流是进程内内存**（[deps.py](server/app/deps.py) 的 `_fails` dict）。
  用多 worker 起 uvicorn 会让限流失效，目前设计就是单进程。
- **`clients.json` 改了不用重启**，按 mtime 自动重载；解析失败会沿用上一版而不是崩掉。
- 目录名是 `webside`（不是 website），别顺手改。
- `server/data/`、`server/.env`、`server/clients.json` 是运行时数据，已在 .gitignore 里。

### 导航数据存在浏览器里

导航卡片存 `localStorage`，按用户分 key（`home-nav:<用户名>`），**没有进后端**，
...
全部集中在 [server/app/config.py](server/app/config.py)，可用环境变量或 `server/.env` 覆盖
